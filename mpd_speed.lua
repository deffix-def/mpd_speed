...
local car_list = {
    ...
    [1205] = "Dodge Viper",
    [1206] = "Ferrari F430",
    [6273] = "Lamborghini Egoista",
    [6717] = "BMW M8",
    [6718] = "BMW X5 (G05)",
    [6719] = "Cadillac CTS",
    [6727] = "Ford F150 Raptor 6x6",
    [6732] = "Koenigsegg CCXR",
    [6739] = "Mercedes-Benz S63 AMG Cabriolet",
    [6740] = "Mercedes-Benz G800 Brabus",
    [6745] = "Land Rover Range Rover III FL"
}
local move = false
local in_veh = false
...
imgui.OnFrame(function()
    return mpds.alpha > 0.00 and (not isGamePaused() and not isPauseMenuActive())
        ...
        if os.time() - update_speed_time >= 1 then
            now_speed = speed
            update_speed_time = os.time()
        end
        if dist == 999 then
            now_speed = u8'Неизвестно'
            dist = u8'Неизвестно'
        else
            if now_speed == u8'Неизвестно' then
                now_speed = speed
            end
        ...
        imgui.SetCursorPosX(3)
        imgui.Text(u8'Скорость: ' .. now_speed)
        imgui.End()
        imgui.PopStyleVar()
    end
)
