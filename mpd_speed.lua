...
local tag = "{FF66FF}[MATools]: {FFFFFF}"
...
function getNearestCarWithDriver()
    ...
    for i, handle in ipairs(getAllChars()) do
        if doesCharExist(handle) and handle ~= PLAYER_PED and isCharInAnyCar(handle) and not isCharOnAnyBike(handle) and not isCharInAnyBoat(handle) and not isCharInAnyHeli(handle) and not isCharInAnyPlane(handle) then
            ...
            if isCharOnScreen(handle) and isLineOfSightClear(mx, my, mz, vx, vy, vz, true, false, false, true, false) then
                if r and cDist < dist then
                    ...
                    model = car_list[getCarModel(storeCarCharIsInNoSave(handle))] or u8'Неизвестно'
                    speed = math.floor(getCharSpeed(handle) * 3.67)
                end
            end
        end
    end
...
function main()
    ...
    sampRegisterChatCommand('mpds_move', function() 
        move = true
        sampAddChatMessage(tag .. 'Перемещайте курсор и нажмите ЛКМ, чтобы сохранить расположение.', 3000)
    end)
    sampRegisterChatCommand('mpds_active', function()
